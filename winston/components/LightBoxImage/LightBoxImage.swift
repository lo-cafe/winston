...
struct LightBoxImage: View {
    ...
    var body: some View {
        ...
        HStack(spacing: SPACING) {
            ForEach(Array(imagesArr.enumerated()), id: \.element.id) { index, img in
                ...
                LightBoxElementView(el: img, onTap: toggleOverlay, doLiveText: doLiveText, isPinching: $isPinching, isZoomed: $isZoomed)
                    ...
                    .offset(x: !selected ? 0 : dragAxis == .vertical ? drag.width : 0, y: i != activeIndex ? 0 : dragAxis == .vertical ? drag.height : 0) //no
            }
        }
        .fixedSize(horizontal: true, vertical: false)
        .offset(x: xPos + (dragAxis == .horizontal ? drag.width : 0)) //no
        ...
        .onAppear {
            if lightboxViewsPost { Task(priority: .background) {
                if let post {
